...
const FeedPage = () => {
  ...
  const [savedVideos, setSavedVideos] = useState([]); // State to store saved videos
  const [isSavedVideosDialogOpen, setIsSavedVideosDialogOpen] = useState(false);
  ...
  const handleSave = () => {
    setSavedVideos((prevSavedVideos) => {
      if (!prevSavedVideos.some(video => video._id === videos[currentIndex]._id)) {
        return [...prevSavedVideos, videos[currentIndex]];
      }
      return prevSavedVideos;
    });
  };

  const handleSavedVideosDialogOpen = () => setIsSavedVideosDialogOpen(true);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <IconButton sx={{ color: "#64b5f6" }} onClick={handleSave}>
          <SaveIcon />
        </IconButton>
        <Button
          variant="contained"
          sx={{ mt: 2 }}
          onClick={handleSavedVideosDialogOpen}
        >
          Saved Videos
        </Button>
      </Box>
      ...
      <Dialog
        open={isSavedVideosDialogOpen}
        onClose={() => setIsSavedVideosDialogOpen(false)}
      >
        <DialogTitle>Saved Videos</DialogTitle>
        <DialogContent>
          <Box>
            {savedVideos.map((video, index) => (
              <Box key={index} sx={{ mb: 2 }}>
                <Typography variant="body1">Video ID: {video._id}</Typography>
                <CardMedia
                  component="video"
                  src={video.videoUrl}
                  controls
        ...
        <DialogActions>
          <Button onClick={() => setIsSavedVideosDialogOpen(false)}>Close</Button>
        </DialogActions>
      </Dialog>
